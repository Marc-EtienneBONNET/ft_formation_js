...
/*   Updated: 2022/07/27 16:44:07 by mbonnet          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	await client1.client1.query(`DELATE FROM raquettes WHERE raquette_id=` + raquette_id + `;`);
}

exports.DeleteOneBall = async function(ball_id)
{
	await client1.client1.query(`DELATE FROM balls WHERE ball_id=` + ball_id + `;`);
}

exports.DeleteOneGame = async function(game_id)
{
	await client1.client1.query(`DELATE FROM games WHERE game_id=` + game_id + `;`);
}
